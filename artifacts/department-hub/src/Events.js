...
export default function Events({ token }) {
  ...
  const [form, setForm] = useState({ title: '', date: '', department: '', attendance: '' });
  ...
  function submit(e) {
    ...
    fetch('http://localhost:4000/api/events', {
      ...
      body: JSON.stringify({ ...form, attendance: Number(form.attendance || 0) })
    })
      .then(r => r.json())
      .then(j => {
        if (j.success) {
      ...
      <ul>
        {events.map(ev => (
          <li key={ev.id} className="event-item">
            <strong>{ev.title}</strong> — {ev.date} ({ev.department}) — {ev.attendance} attendees
          </li>
        ))}
      </ul>
